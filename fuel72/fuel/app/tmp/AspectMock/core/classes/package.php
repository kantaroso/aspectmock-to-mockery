...
class Package
{
	...
	{ if (($__am_res = __amock_before(get_called_class(), __CLASS__, __FUNCTION__, array($package, $path), true)) !== __AM_CONTINUE__) return $__am_res; 
		if (is_array($package))
		{
			$result = true;
	...
	{ if (($__am_res = __amock_before(get_called_class(), __CLASS__, __FUNCTION__, array($package), true)) !== __AM_CONTINUE__) return $__am_res; 
